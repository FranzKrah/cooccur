cooccur_krah <- function (mat = finches, 
  type = "spp_site",
  thresh = TRUE,
  spp_names = FALSE,
  ...
  eff_matrix = FALSE) 
{
  ...
  incidence <- prob_occur <- matrix(nrow = nrow(N_matrix), 
    ncol = ncol(N_matrix))
  ## compute number of sites species i occures in off-diagonals
  # for (spp in 1:nspp) {
  #   if (spp < nspp) {
  #     for (spp_next in (spp + 1):nspp) {
  #       # off-diagonals: number of sites species i occures
  #       incidence[spp, spp_next] <- sum(site_mask[spp, ] * site_mask[spp_next, ] * mat[spp, ])
  #       # diogonals: number of sites species i+1 occures on
  ...
  #### BEGIN REPLACEMENT ####
  incidence <- do.call(rbind, lapply(rowSums(mat), rep, nrow(mat)))
  diag(incidence) <- NA
  combis <- combn(nrow(mat), 2, FUN = NULL, simplify = TRUE)

  require("qdapTools")
  occ <- v_outer(t(mat), function(x, y)  sum(x==1 & y==1))
  diag(occ) <- 0
  rownames(occ) <- colnames(occ) <- NULL
  
  # probability that species x either occures (diagonal) or occures together with species y (off-diagonal)
  prob_occur <- incidence/N_matrix  
  rownames(N_matrix) <- colnames(N_matrix) <- rownames(incidence)
  ...
  #   if (spp < nspp) {
  #     for (spp_next in (spp + 1):nspp) {
  #       pairs <- sum(as.numeric(mat[spp, site_mask[spp, ] * site_mask[spp_next, ] == 1] == 1
  ...
  #### BEGIN REPLACEMENT ####
  obs_cooccur <- matrix(ncol = 3, nrow = ncol(combis))
  for(i in 1:ncol(combis)){
    combi <- combis[,i]
    obs_cooccur[i,1] <- combi[1]
    obs_cooccur[i,2] <- combi[2]
  }
  prob_cooccur <- exp_cooccur <- obs_cooccur
  
  ## matrix with co-occurrence numbers
  for(i in 1:ncol(combis)){
    combi <- combis[,i]
    obs_cooccur[i,3] <- occ[combi[2], combi[1]]
  }
  ## matrix with co-occurrence prob
  ## conditional probability that species x occurs given species y occures
  for(i in 1:ncol(combis)){
    combi <- combis[,i]
    prob1 <- prob_occur[combi[2], combi[1]]
    prob2 <- prob_occur[combi[1], combi[2]]
    prob_cooccur[i,3] <- (prob1 * prob2)
  }
  ## matrix with expected co-occurrence
  for(i in 1:ncol(combis)){
    combi <- combis[,i]
    exp_cooccur[i,3] <- prob_cooccur[i, 3] * N_matrix[combi[1], combi[2]]
  }
  ####### END REPLACEMENT ########
